...
class Panier extends Component {
	...
	render() {
		...
		function ItemImages(item_id) {
			...
			return cartItemsImages[indexOf_cartitem_images]?.cartitem_images?.images[0]
		}
													...
													<center className="pb-5 ">
														<center class="lds-spinner float-left "><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div><div></div></center>
															...
															<Badge badgeContent={"Personnalisé "} color="primary">
																<img onClick={() => this.props.CustomPackDetail(cartitem.item.id ,ItemImages(cartitem.cartitem_id))} className="ml-2" alt="" src={ItemImages(cartitem.cartitem_id)} />
															</Badge>

														

													) :
														<Link to={{ pathname: `/item/${cartitem.item.type}/${cartitem.item.id}`, search: `` }} onClick={() => window.location.href = `/item/${cartitem.item.type}/${cartitem.item.id}`}>
