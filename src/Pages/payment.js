...
class Address extends Component {
    ...
    async componentDidMount() {
        this.update();
        try {
            const userInfo = await axiosInstance.get(`api/user/detail`)
            const cartDetails = await axiosInstance.get(`api/cart/detail`)

            this.setState({
                name: userInfo.data.name,
                second_name: userInfo.data.second_name,
    ...
    async SaveClientInfo(e) {
        ...
        if (this.state.phone_number.length < 10) {
        ...
        else if (this.state.second_name.length === 0) {
            this.setState({ second_name_err: 'Champs requis', is_second_name_err: true, isDataSaving: false })
        }
            ...
            const [packUserImages] = await Promise.all([
                axiosInstance.put(`api/user/detail`,
                    {
                ...
                axiosInstance.put(`api/user/address/detail`,
                    {
    ...
    render() {
        ...
        const { name, second_name, phone_number, address, city, isDataLoading } = this.state
                                    ...
                                    <form onSubmit={this.SaveClientInfo} className="bg-white pt-0">
                                        ...
                                        <div className="pl-5 pr-5   ">
                                            <div className=" row justify-content-between ">
                                                <TextField className="col-6 ml-1 mb-4  " error={is_name_err} helperText={name_err} id="name" value={`${name}`} onChange={this.onChange} label="Nom*" />
                                                <TextField className="mb-4" id="second_name" label="Prenom*" error={is_second_name_err} helperText={is_second_name_err} value={`${second_name}`} onChange={this.onChange} />
                                            </div>
